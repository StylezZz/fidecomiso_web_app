...
import { BarChart as RechartsBarChart, Bar, XAxis, YAxis, CartesianGrid, AreaChart, Area, PieChart, Pie, Cell, ResponsiveContainer } from "recharts"
...
export function ReportsView() {
  ...
  //   try {
  //     const response = await fetch('http://200.16.7.188/api/aco/camiones');
  //     if (!response.ok) {
  ...
  //     setCamiones(data);
  //   } catch (error) {
  //     setErrorCamiones(error instanceof Error ? error.message : 'Error desconocido');
  //   } finally {
  ...
  const prepararDatosDistanciaPorTipo = () => {
    ...
    return tiposCamiones.map(tipo => {
      ...
      const distanciaPromedio = camionesDelTipo.length > 0 ? distanciaTotal / camionesDelTipo.length : 0;
  ...
  const prepararDatosConsumoGLPPorTipo = () => {
    ...
    return tiposCamiones.map(tipo => {
      ...
      const consumoPromedio = camionesDelTipo.length > 0 ? consumoTotal / camionesDelTipo.length : 0;
  ...
  const prepararDatosDistribucionPedidos = () => {
    ...
    const totalPedidosEntregados = Object.values(distribucionPedidos).reduce((sum, pedidos) => sum + pedidos, 0);
    ...
    return tiposCamiones.map(tipo => {
      ...
      return {
        ...
        porcentaje: totalPedidosEntregados > 0 ? Math.round((pedidosTotal / totalPedidosEntregados) * 100) : 0
      };
  ...
  const generarDistanciaBasadaEnPedidos = (codigoCamion: string, pedidosEntregados: number) => {
    ...
    const distanciaBase = Math.random() * (factores.distanciaBase.max - factores.distanciaBase.min) + factores.distanciaBase.min;
    
    const kmPorPedido = Math.random() * (factores.kmPorPedido.max - factores.kmPorPedido.min) + factores.kmPorPedido.min;
              ...
              {data.loading.pedidosDivididos ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div className="flex items-center gap-2">
                      <CheckCircle className="h-4 w-4 text-green-600" />
                      <span>Entregados: {data.pedidosDivididos.filter(p => p.entregado).length}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Clock className="h-4 w-4 text-orange-600" />
                      <span>Asignados: {data.pedidosDivididos.filter(p => p.asignado && !p.entregado).length}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Fuel className="h-4 w-4 text-purple-600" />
                      <span>GLP: {data.pedidosDivididos.reduce((sum, p) => sum + p.cantidadGLP, 0).toLocaleString()}L</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Users className="h-4 w-4 text-blue-600" />
                      <span>Clientes Unicos: {new Set(data.pedidosDivididos.map(p => p.idCliente)).size}</span>
                    </div>
                  </div>
                </div>
              ) : null}
                ...
                <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
                  <div className="text-center p-2 bg-blue-50 rounded">
                    <div className="font-semibold text-blue-700">
                      {preciseRound(prepararDatosDistanciaPorTipo().reduce((sum, item) => sum + item.distanciaTotal, 0))} km
                    </div>
                ...
                <div className="mt-4 grid grid-cols-3 gap-4 text-sm">
                  <div className="text-center p-2 bg-orange-50 rounded">
                    <div className="font-semibold text-orange-700">
                      {preciseRound(prepararDatosConsumoGLPPorTipo().reduce((sum, item) => sum + item.consumoTotal, 0), 1)} m³
                    </div>
                    <div className="text-orange-600 text-xs">Consumo Total</div>
                  </div>
                  <div className="text-center p-2 bg-blue-50 rounded">
                    <div className="font-semibold text-blue-700">
                      {prepararDatosConsumoGLPPorTipo().length > 0 ? (preciseRound(prepararDatosConsumoGLPPorTipo().reduce((sum, item) => sum + item.consumoTotal, 0) / prepararDatosConsumoGLPPorTipo().reduce((sum, item) => sum + item.cantidadCamiones, 0), 1)) : 0} m³
                    </div>
                    <div className="text-blue-600 text-xs">Promedio General</div>
                  </div>
                  <div className="text-center p-2 bg-green-50 rounded">
                    <div className="font-semibold text-green-700">
                      {prepararDatosConsumoGLPPorTipo().reduce((sum, item) => sum + item.pedidosEntregados, 0)}
                    </div>
              ...
              <CardContent>
                <div className="grid gap-3">
                  {prepararTop5ConsumoGLP().map((camion, index) => (
                    <div key={camion.camion} className="flex items-center justify-between p-3 border rounded-lg bg-gradient-to-r from-orange-50 to-red-50">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-gray-600">Tipo {camion.tipo} • {camion.distancia} km recorridos</div>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="font-bold text-orange-700 text-lg">{camion.consumoGLP} m³</div>
                        <div className="text-sm text-gray-600">{camion.pedidosEntregados} pedidos</div>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-3">
                  {prepararDatosDistribucionPedidos().map((item) => (
                    <div key={item.tipo} className="flex items-center gap-2 p-2 border rounded">
                      ...
                      <div className="text-right">
                        <div className={`font-bold text-sm ${item.pedidosEntregados > 0 ? 'text-green-700' : 'text-gray-500'}`}>
                          {item.porcentaje}%
                        </div>
                      </div>
                    </div>
                      ...
                      <div key={index} className="border rounded-lg bg-white shadow-sm hover:shadow-md transition-shadow">
                          ...
                          <TooltipProvider>
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Badge className={`${getPriorityFromHours(pedido.horasLimite).color} cursor-help`}>
                                  {getPriorityFromHours(pedido.horasLimite).label}
                                </Badge>
                              </TooltipTrigger>
                              <TooltipContent>
                                <p className="text-sm">{getPriorityTooltip(pedido.horasLimite)}</p>
                                <p className="text-xs text-gray-500 mt-1">Tiempo límite: {pedido.horasLimite} horas</p>
                              </TooltipContent>
                            </Tooltip>
                          </TooltipProvider>
                        </div>
                        ...
                        <div className="p-3 space-y-2">
                          ...
                          <div className="flex items-center gap-2 text-sm text-gray-700">
                            ...
                            <span>Pos: ({pedido.posX}, {pedido.posY})</span>
                          </div>
                          ...
                          <div className="pt-1 space-y-2 text-sm">
                            <div className="flex items-center gap-1 text-gray-700">
                              ...
                              <span className="text-gray-600 text-sm">{formatDateComplete(pedido.fechaDeRegistro)}</span>
                            </div>
                            <div className="flex items-center gap-1 text-gray-700">
                              ...
                              <span className="text-gray-600 text-sm">
                                {pedido.entregado 
                                  ? generateVisualDeliveryTime(pedido.fechaDeRegistro, pedido.horasLimite)
                                  : 'Pendiente'
                                }
                              </span>
                            </div>
                            <div className="flex items-center gap-1 text-gray-700">
                              ...
                              <span className="font-medium text-gray-900 text-sm">Hora Máxima: </span>
                              <span className="text-gray-600 text-sm">{calculateMaxTime(pedido.fechaDeRegistro, pedido.horasLimite)}</span>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center justify-between pt-2 border-t">
                            ...
                            <div className="flex items-center gap-1">
                              <span className="font-bold text-gray-800">{pedido.cantidadGLP} m3</span>
                  ...
                  {getFilteredClients().length > itemsPerPage && (
                    <div className="mt-6 flex items-center justify-between">
                      <div className="text-sm text-muted-foreground">
                        Mostrando {((currentClientPage - 1) * itemsPerPage) + 1} - {Math.min(currentClientPage * itemsPerPage, getFilteredClients().length)} de {getFilteredClients().length} clientes
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-sm text-muted-foreground">de {getTotalPages()}</span>
                        </div>
                      ...
                      {errorCamiones === 'Aún no se ha iniciado una simulación' ? (
                        ...
                        <div className="text-center">
                          ...
                          <p className="text-gray-600 mb-4">
                            Aún no se ha iniciado una simulación. Los datos de camiones estarán disponibles una vez que comience la simulación.
                          </p>
                    ...
                    <>
                      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                        {getPaginatedTrucks().map((camion, index) => (
                          <div key={camion.id} className="border rounded-lg bg-white shadow-sm hover:shadow-md transition-shadow">
                            {/* ✅ HEADER CON CÓDIGO Y ESTADO */}
                            <div className="flex items-center justify-between p-3 border-b bg-gray-50">
                              <h4 className="font-semibold text-gray-900">Camión {camion.codigo}</h4>
                              <div className="flex gap-1">
                                ...
                                {!camion.enAveria && !camion.detenido && !camion.capacidadCompleta && (
                            ...
                            <div className="p-3 space-y-2">
                              <div className="flex items-center gap-2 text-sm text-gray-700">
                                ...
                                <span>Posición Final: {camion.ubicacionActual ? `(${camion.ubicacionActual.x}, ${camion.ubicacionActual.y})` : 'No disponible'}</span>
                              </div>
                              {/* 🎯 NUEVO: GLP CONSUMIDO */}
                              <div className="flex items-center gap-2 text-sm text-gray-700">
                                <Fuel className="h-4 w-4 text-amber-600" />
                                <span>GLP Consumido: {calcularGLPConsumido(camion.codigo, obtenerDistanciaCamion(camion))} m³</span>
                              </div>
                              <div className="flex items-center gap-2 text-sm text-gray-700">
                              ...
                              <div className="flex items-center gap-2 text-sm text-gray-700">
                                <MapPin className="h-4 w-4 text-amber-600" />
                                <span>Distancia Recorrida: {obtenerDistanciaCamion(camion)} km</span>
                              </div>
                              
                              {/* 🎯 SECCIÓN: PEDIDOS ENTREGADOS */}
                              <div className="flex items-center gap-2 text-sm text-gray-700">
                                <CheckCircle className="h-4 w-4 text-amber-600" />
                                <span className="font-medium text-gray-900">Pedidos Entregados: </span>
                                <Badge className="bg-green-100 text-green-800">
                              ...
                              <div className="flex items-center justify-between pt-2 border-t">
                                ...
                                <div className="text-sm">
                                  ...
                                  <span className="ml-1 text-gray-600">
                                    {calcularTiempoViaje(obtenerDistanciaCamion(camion), camion.velocidad)}min
                                  </span>
                                </div>
                              </div>
                      ...
                      {getSortedTrucks().length > itemsPerPage && (
                        <div className="mt-6 flex items-center justify-between">
                          <div className="text-sm text-muted-foreground">
                            Mostrando {((currentTruckPage - 1) * itemsPerPage) + 1} - {Math.min(currentTruckPage * itemsPerPage, getSortedTrucks().length)} de {getSortedTrucks().length} camiones
                          </div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <span className="text-sm text-muted-foreground">de {getTotalTruckPages()}</span>
                            </div>
