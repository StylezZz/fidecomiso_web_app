...
export const BlockRoute = ({ block,setBloqueoSeleccionado,setToolTipBlockPos,onTooltip }: Props) => {
...
export const ToolTipBlockRoute = ({block,posX,posY}: ToolTipBlockRouteI)=>{
  ...
  const timeMinsBlock = dateFinBlock.getDate()*1440 + dateFinBlock.getHours()*60 + dateFinBlock.getMinutes();
  ...
  for(let tramo of block.tramo){
    ...
    if(x_fin === x_ini)
      distance += Math.abs(y_fin-y_ini);
    if(y_fin === y_ini)
      distance += Math.abs(x_fin-x_ini);
  }
