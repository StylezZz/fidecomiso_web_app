"use client";
import StoreHouse, { AlmacenInfo, ToolTipAlmacen } from "@/components/map/almacen/Almacen";
...
const MapCanvas = forwardRef<MapCanvasRef, MapProps>(({ open }, ref) => {
  ...
  // Estado para controlar si el panel está desplegado
  ...
  const obtenerPedidosEntregadosAPartirDeVehiculos = useCallback(
    (vehiculos: VehiculoI[], timer: number) => {
      let pedidos: number[] = [];
  ...
  const pedidosEntregados = useMemo(
    () => obtenerPedidosEntregadosAPartirDeVehiculos(dataVehiculos, timerSimulacion),
    [dataVehiculos, obtenerPedidosEntregadosAPartirDeVehiculos, timerSimulacion]
  );
    ...
    <>
      <div className="flex flex-1 h-full">
        ...
        >
          ...
          {/* Botón de toggle - posicionado sobre el área del mapa */}
          <button
            onClick={togglePanel}
            className="
              absolute top-4 right-4 z-10 bg-white border border-gray-300 rounded-md shadow-md 
        ...
        {/* Panel lateral - MapTable que empuja el contenido */}
        <div
          className={`
          ...
          `}
        >
          <div className={`w-80 h-full ${isPanelExpanded ? "block" : "hidden"}`}>
