...
class PedidosService {
    ...
    public static async getOrders(days?: number[], year?:number, month?: number): Promise<HttpResponse> {
    ...
    public static async postReadOrdersFile(file : File) : Promise<HttpResponse>{
        ...
        try{
            const res = await http.post(`/pedidos/leer-pedidos`,formData);
    ...
    public static async getNombrePedidosCargados() : Promise<HttpResponse>{
        try{
            const res = await http.get("/pedidos/nombre-pedidos-archivos");
            if(!res.success)throw new Error(res.error);
            return res;
        }
        catch(error){
