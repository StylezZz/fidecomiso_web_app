...
export const useManageTime = ({tipo,setSimulacionIniciada,simulacionIniciada}:manageTimeProps): manageTimeReturn => {
    ...
    const startTimer =  async() => {
        ...
        if(simulacionIniciada && !initTimer){ //si ya se iniicio y está en pausa, se inica
            setInitTimer(true);
            return;
        }
        ...
        try{
            ...
            const response = await SimulationService.inicializarTipoSimulacion(tipo== SimulationType.DIA_DIA ? 1 : tipo== SimulationType.SEMANAL ? 2 : 3);
